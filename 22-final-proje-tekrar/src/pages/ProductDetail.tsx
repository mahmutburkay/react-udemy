...
function ProductDetail() {
    ...
    const { productId } = useParams(); // id yi useParams'la yakaladık 
    const dispatch = useDispatch();

    const [count, setCount] = useState<number>(0);


    //* İD Sİ OLAN ÜRÜNÜ GETİR 
    ...
    const getProductById = async (productId: number) => {
        try {
            ...
            setProduct(product); // state in içine verdik
        } catch (error) {
                    ...
                    <div style={{ marginLeft: '60px', marginTop: '60px' }}>
                        ...
                        <div>
                            <Button onClick={addBasket}
                                color='info' variant='contained' size='small' sx={{ textTransform: 'none', marginTop: '20px' }}>Sepete Ekle</Button>
                        </div>
                    </div>
                </div>
